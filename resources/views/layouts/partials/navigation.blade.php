        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('notifications.center') }}" 
               class="relative group rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 p-2.5 text-slate-700 hover:from-slate-200 hover:to-slate-300 transition-all duration-200">
                ...
                <span id="global-notification-badge"
                      class="absolute -top-1 -right-1 {{ $unreadCount > 0 ? 'flex' : 'hidden' }} h-5 w-5 items-center justify-center rounded-full bg-[var(--nmis-accent)] text-[10px] font-bold text-white shadow-lg animate-pulse">
                    <span id="global-notification-count">{{ $unreadCount > 9 ? '9+' : $unreadCount }}</span>
                </span>
            </a>
